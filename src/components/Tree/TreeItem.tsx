    ...
    getIcon(node: any) {
        ...
        if (node.icon) {
            ...
            icon = node.expanded && node.children && node.children.length ?
                (node.expandedIcon || (<svg width="24" height="24" viewBox="0 0 24 24" fill="#757575">
                    <path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/>
                </svg>)) : (node.collapsedIcon || (<svg width="24" height="24" viewBox="0 0 24 24" fill="#757575">
                    <path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/>
                </svg>));
        }
        if (this.isSelected()) {
            icon = (<svg width="24" height="24" viewBox="0 0 24 24" fill="#4285F4">
                       ...
                       {tree && tree.props.selectionMode === 'checkbox' ? (<div className={`${prefixCls}-checkbox`}>
                           <div className={`${prefixCls}-checkbox-box`}>
                               <div className={`${prefixCls}-checkbox-icon`}>
                                   {node.partialSelected ? (<svg fill="#4285F4" height="24" viewBox="0 0 24 24" width="24">
                                       <defs>
                ...
                {node.children && node.expanded ? (<ul className={`${prefixCls}-children`} style={expandedStyle}>
                    {
                        node.children.map((childNode: any, index: number) => {
                            return (<TreeItem selection={selection} tree={tree} node={childNode} parentNode={node}
                                              key={`TreeNode-${index}`} index={index + this.props.index!}/>);
                        })
                    }
                </ul>) : null}
