...
export default class IndexList extends React.Component<IndexListProps, IndexListState> {
    ...
    getOffsetTop(element: any, condition: Function) {
    ...
    swipeMove = (event: any) => {
        ...
        if (this.dragging) {
            this.scrollTo(event);
            let {y} = this.getPoint(event);
            let diffY = y - this.touches.startY + this.state.scrollTop!;
            this.setState({
                    ...
                    <ul>
                        {
                            this.splitLetter().map((letter: string, index: number) => {
                                return (
                                    <li className={`${prefixCls}-nav-item`} data-index={letter} key={index}>{letter}</li>
                                )
                            })
                        }
                    </ul>
