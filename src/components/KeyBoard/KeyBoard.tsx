...
export default class KeyBoard extends React.Component<KeyBoardProps, KeyBoardState> {
    ...
    handleClick = (num: number) => {
        ...
        this.setState({
            ...
            if (this.props.onFinish) {
                (this.props.onFinish as Function)(this.state.password);
            }
        });
    };
    ...
    getNumbersSpan() {
        ...
        this.numbers.forEach((num: number) => {
            children.push(<span key={`numbers-${num}`}
                                onClick={this.handleClick.bind(this, num)}>{num}</span>);
        });
        children.push(<span key={`pwd-cancel`} onClick={this.handleClose}>取消</span>);
        children.push(<span key={`pwd-zero`}
                            onClick={this.handleClick.bind(this, 0)}>0</span>);
